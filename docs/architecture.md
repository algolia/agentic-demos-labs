...
Check the `/assistant/` directory for a vanilla JavaScript implementation that can be embedded in any HTML page with `<script>` tags. It uses the same Agent Studio API but without React dependencies.
